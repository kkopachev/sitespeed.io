...
module.exports = {
  send(options, group, url, baseDir, pagePath, time) {
    ...
    const tags0_9 =  `"${connectivity},${browser},${namespace.join(',')},${urlAndGroup.join(',')}"`;
    const tags1_0 = `["${connectivity}","${browser}","${namespace[0]}","${namespace[1]}","${urlAndGroup[0]}", "${urlAndGroup[1]}"]`;
    ...
    const tags = options.graphite.use1_0 ? tags1_0 : tags0_9;
    const postData =
      `{"what": "Sitespeed.io", "tags": ${tags}, "data": "${message}", "when": ${timestamp}}`;
    const postOptions = {
      hostname: 'graphite-web', // !!!HARDCODED!!!
